...
module.exports.socketIO = function(server, stores) {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("message", auth, (msg) => message(socket, msg, users));

        socket.on("order-bundles", auth, (data) => orderBundles(socket, data, getUserRoomsAndRole(socket.id)[0]));

        socket.on("order-status", auth, (data) => orderStatus(socket, data, getUserRoomsAndRole(socket.id)[0]));

        socket.on("driver-status", auth, (status) => driverStatus(socket, status, users[socket.id], getUserRoomsAndRole(socket.id)[0]));

        socket.on("position", auth, (positionObj) => position(socket, positionObj));

        socket.on("disconnect", auth, (reason) => {
...
function findUserSocket(userId) {
    return Object.keys(users).find(socketId => users[socketId] == userId)
}
